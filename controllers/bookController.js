      ...
      book_bookinstances: function(callback) {
        BookInstance.find({'book': req.params.id}).exec(callback);
         ...
         if(results.book_bookinstances.length > 0) {
           // Book has no BookInstance. Render in same way as for GET route.
           res.render('book_delete', {title: 'Delete Book', book: results.book, book_instances: results.book_bookinstances })
           return;
      }
      else {
        // Book has no bookinstances objects. Delete the object and redirect to book list
        Book.findByIdAndRemove(req.body.id, function deleteBook(err) {
          if(err) {return next(err); }
          // Successful go to list_books
...
exports.book_update_post = [
   ...
   // Validate fields
   body('title', 'Title mus not be empty').isLength({min: 1}).trim(),
   body('author', 'Author must not be empty').isLength({min: 1}).trim(),
   body('summary', 'Summary must not be empty').isLength({min: 1}).trim(),
   body('isbn', 'ISBN must not be empty').isLength({min: 1}).trim(),
